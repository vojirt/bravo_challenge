...
class DINOv2Augmentation():
    ...
    def train(self, cfg):
        augment = Compose([
            ToTensorSN(),
            ResizeLongestSideDivisible(cfg.INPUT.IMG_SIZE, cfg.MODEL.PATCH_SIZE, randomcrop=cfg.INPUT.RANDOMCROP_AUG, hflip=cfg.INPUT.HFLIP_AUG, fix_aspect=cfg.INPUT.FIX_ASPECT),
    ...
    def val(self, cfg):
        augment = Compose([
            ToTensorSN(),
            ResizeLongestSideDivisible(cfg.INPUT.IMG_SIZE, cfg.MODEL.PATCH_SIZE, eval_mode=False, randomcrop=False, fix_aspect=cfg.INPUT.FIX_ASPECT),
            NormalizeSN(mean=cfg.INPUT.NORM_MEAN, std=cfg.INPUT.NORM_STD)
        ])
        return augment
    ...
    def test(self, cfg):
        augment = Compose([
            ToTensorSN(),
            ResizeLongestSideDivisible(cfg.INPUT.IMG_SIZE, cfg.MODEL.PATCH_SIZE, eval_mode=True, randomcrop=False, fix_aspect=False),
            NormalizeSN(mean=cfg.INPUT.NORM_MEAN, std=cfg.INPUT.NORM_STD)
        ])
        return augment
