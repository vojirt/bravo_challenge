...
class ResizeMaxSN():
    ...
    def __call__(self, x_sn):
        x_size = [x_sn.image.height, x_sn.image.width]
        if x_size[0] >= x_size[1]:
            factor = x_size[0] / float(self.max_size)
            size = [int(self.max_size), int(x_size[1] / factor)] 
        else:
            factor = x_size[1] / float(self.max_size)
            size = [int(x_size[0] / factor), int(self.max_size)] 

        x_sn.image = torchvision.transforms.functional.resize(x_sn.image, size=size)
        if x_sn.label is not None:
            x_sn.label = torchvision.transforms.functional.resize(x_sn.label, size=size, interpolation=InterpolationMode.NEAREST)
        return x_sn 
...
class ResizeLongestSideDivisible():
    def __init__(self, img_size, divider, eval_mode=False, randomcrop=False, hflip=False, fix_aspect=False):
