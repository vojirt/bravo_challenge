...
def patch_label_distribution(target, patch_size, NUM_CLASSES):
    # [B, L, Hf, Wf]
    counts_per_label = torch.nn.functional.interpolate(
                            torch.nn.AvgPool2d((patch_size, patch_size))
                                (
                                   torch.stack([(target == ul).float() for ul in range(0, NUM_CLASSES)], dim=1)
                                ), 
                            size=[target.shape[-2]//patch_size, target.shape[-1]//patch_size], mode="bilinear", align_corners=True) 

    # [B * Hf * Wf, C]
    norm = torch.sum(counts_per_label, dim=1, keepdim=True)
    norm[norm == 0] = 1e-9
    weights = rearrange(counts_per_label / norm, "b c h w -> (b h w) c") 
    return weights
